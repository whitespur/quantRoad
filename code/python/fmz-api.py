...
def main():
    ...
    #     Log('%s => %s' % (item, ret))
    # testExchanges = [
    #     'GetName', 'GetUSDCNY', 'GetRate', ['SetRate', 1.0], 'GetAccount', 'GetCurrency', 'GetLabel', 'GetMinStock',
    #     'GetMinPrice', 'GetFee',
    #     'GetRecords',
    #     ['Buy', 100, 0.1, "OK"],
    #     'GetOrders',
    ...
    Log("EMA len", len(TA.EMA(exchange.GetRecords())))
    Log("EMA H1 len", len(TA.EMA(exchange.GetRecords(PERIOD_H1))))

    Log("EMA", TA.EMA(exchange.GetRecords()))
    Log("EMA H1", TA.EMA(exchange.GetRecords(PERIOD_H1)))
    Log("BOLL", TA.BOLL(exchange.GetRecords,20,2))
